...
def get_trending_topics():
    html = get_eksi_page("https://eksisozluk.com/basliklar/gundem")
...
def search_topic(query, page=1):
    ...
    url = f"https://eksisozluk.com/?q={query}"
    logging.info(f"Searching for: {url}")

    return get_topic(url, page)
...
def get_entries_from_url(url):
    ...
    if entries_ul:
        ...
        for entry_li in entries_li:
            ...
            entries.append(
                {"id": id, "author": author, "author_id": author_id, "content": content}
            )
    return entries


if __name__ == "__main__":
    page = search_topic("elon musk", 1)
    entries = get_entries_from_url(page["url"])
